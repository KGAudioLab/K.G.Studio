...
- 推薦硬件：至少 **8 GB 顯存**的 GPU，或至少 **16 GB 統一內存**的系統。
...
- 在整個流程中，保持您希望 agent 更新的 MIDI 區域處於選中狀態。
- 僅僅為了執行 `read_music`，您不需要切換到別的區域，因為 `read_music` 可以讀取所有音軌的音樂內容。

### 改用外部 LLM
- 如果您想使用更大的雲端模型或自托管模型，請打開 **設置 -> 通用 -> LLM 提供方**，切換到 **本地 LLM（瀏覽器）** 以外的提供方。
- 如果要用雲端模型，可以選擇 **OpenAI**，或者選擇 **OpenAI Compatible** 並填寫 OpenRouter 等提供方。
- 如果要用自托管模型，也請選擇 **OpenAI Compatible**，然後填寫你的服務器 **Base URL** 和 **Model**。
