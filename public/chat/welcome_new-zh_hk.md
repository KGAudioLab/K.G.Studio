...
- 進入 **設置 ⚙️ → 通用 → LLM 提供方**，選擇 **OpenAI 兼容服務**。
- 在 **OpenAI 兼容服務 → 密鑰** 中粘貼您的 Key。注意：在非 localhost 環境下，Key 默認不會持久化，刷新後需要重新輸入，請妥善保管。
...
**基本 DAW 操作**
- 在音軌上雙擊（或按住 Ctrl/Cmd 再點擊）可創建區域。
- 拖動區域邊緣可調整長度；拖動區域主體可移動。
- 點擊區域左上角的小鉛筆可打開鋼琴卷簾窗口。
- 在鋼琴卷簾中，雙擊（或 Ctrl/Cmd+點擊）可創建音符。
- 單擊選擇；Shift+單擊多選；拖拽框選。
- 拖動音符邊緣可調整長度；拖動音符主體可移動所選音符。
- 使用鋼琴卷簾工具欄右上角的 Snapping 控制網格量化。

### 免責聲明
K.G.Studio 與任何 LLM 提供方均無隸屬關係。所有數據默認都保存在您的本地設備中；您發送給第三方服務的數據由您自行負責。
