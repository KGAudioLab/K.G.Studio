...
export const zhCnMessages: TranslationMessages = {
  ...
  'settings.audioIo.description': '选择 KGStudio 用于录音和播放的设备。输入设备的更改会在下一次录音时生效。输出设备的更改在 v1 中需要刷新页面后生效。',
  ...
  'settings.audioIo.outputHelp': '在 v1 中，输出设备更改需要刷新页面。浏览器对非默认输出路由的支持有限，因此在不支持时 KGStudio 会继续使用系统默认设备。',
  ...
  'settings.audioIo.status.outputSavedRefreshRequired': '音频输出设备已保存。在 v1 中请刷新页面以应用。',
  'settings.audioIo.status.outputPromptUnsupported': '此浏览器不支持弹出音频输出设备选择。',
  'settings.audioIo.status.outputSelectedRefreshRequired': '已选择输出设备。请刷新页面以在 v1 中应用。',
  ...
  'settings.behavior.midiInputLatencyHelp': '用于 MIDI 录音的时间校正（0-500ms）。如果录下来的音符相比你实际想要演奏的位置略晚，可以增加此值以匹配你的 MIDI 设备输入延迟。每个音符在提交时都会向前回移这段时间。',
  ...
  'settings.templates.placeholder': '请输入你为 K.G.Studio 音乐创作助手准备的自定义指令',
